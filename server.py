...
PUNTOS_REFERENCIA = {
    ...
    'FARDARIAS': {
        ...
        'factor_dia': 6,
        'fuerza_modificador': 1,
        'descripcion': 'Sistema de periodización temporal basado en el ciclo de los 7 planetas tradicionales. Las fardarias representan periodos determinados por los números planetarios que revelan influencias específicas en diferentes épocas de la vida.'
    }
}
...
def calculate_positions_with_skyfield(utc_datetime, lat=None, lon=None):
    ...
    try:
        ...
        if lat is not None and lon is not None:
            ...
            # Calcular y añadir Parte de Fortuna y Parte del Espíritu
...
# 1. Función calculate_date (debe definirse antes de usarla en calculate_fardaria_periods)
def calculate_date(birth_date, day_offset):
    """Calcula una fecha a partir de una fecha base y un desplazamiento en días."""
    # Si birth_date ya es un objeto datetime, usarlo directamente
    if isinstance(birth_date, datetime):
        base_date = birth_date
    else:
        # Intentar convertir la cadena a datetime
        try:
            base_date = datetime.strptime(birth_date, '%Y-%m-%d')
        except ValueError:
            try:
                # Probar otro formato
                base_date = datetime.strptime(birth_date, '%d/%m/%Y')
            except ValueError:
                # Si todo falla, crear un datetime a partir de una fecha fija
                print(f"Error al convertir fecha: {birth_date}, usando fecha predeterminada")
                base_date = datetime(2000, 1, 1)
    
    # Calcular la nueva fecha
    result_date = base_date + timedelta(days=int(day_offset))
    
    # Devolver como cadena en formato día/mes/año
    return result_date.strftime('%d/%m/%Y')

# 4. Función calculate_sub_periods
...
# 5. Función calculate_fardaria_periods (versión completa)
def calculate_fardaria_periods(birth_date, is_dry, start_year=None, end_year=None):
    ...
    try:
        ...
        while current_day < end_day:
            for planet in planet_order:
                ...
                if period_duration > 0:
                    start_date = calculate_date(birth_date_obj, period_start_day)
                    end_date = calculate_date(birth_date_obj, period_end_day)
                    
                    # Obtener el mes para determinar el signo y regente
                    try:
                        start_date_obj = datetime.strptime(start_date, '%d/%m/%Y')
                        month = start_date_obj.month
                    except:
        ...
        print(f"Error en calculate_fardaria_periods: {e}")
        import traceback
        traceback.print_exc()
        return []  # Devolver lista vacía en caso de error

# 3. Función calculate_duration
def calculate_duration(planet, level):
    """Calcula la duración de un periodo de Fardaria."""
    number = PLANET_DATA[planet]['numero']
    if level == 1:
...
def calculate_forecasts():
    ...
    try:
        ...
        # Validar punto de referencia - Modificado para aceptar FARDARIAS
        valid_refs = list(PUNTOS_REFERENCIA.keys())
        if punto_referencia and punto_referencia not in valid_refs:
            return jsonify({"error": f"Punto de referencia '{punto_referencia}' no válido. Opciones: {', '.join(valid_refs)}"}), 400
        ...
        if punto_referencia == 'FARDARIAS':
            fardarias = calculate_fardaria_periods(birth_date, is_dry, start_year, end_year)
        else:
            # Para otros puntos de referencia, también calculamos fardarias por compatibilidad
            fardarias = []
            try:
                fardarias = calculate_fardaria_periods(birth_date, is_dry, start_year, end_year)
            except Exception as e:
                print(f"Error calculando fardarias (no crítico): {e}")
        ...
        # Para el punto de referencia FARDARIAS, creamos "liberaciones" basadas en los periodos de fardarias
        if punto_referencia == 'FARDARIAS':
            liberaciones = []
            for periodo in fardarias:
                # Creamos una liberación por cada cambio de periodo principal
                liberaciones.append({
                    "fecha": periodo['start'],
                    "planeta_liberador": periodo['planet'],
                    "planeta_liberado": "TODOS",  # Esto es arbitrario
                    "punto": "FARDARIAS",
                    "angulo": 0,  # Valor arbitrario
                    "fuerza": PLANET_DATA[periodo['planet']]['numero'],
                    "signo": periodo['sign'],
                    "regente": periodo['regente'],
                    "condicion": "Inicio de Periodo",
                    "descripcion": f"Inicio de período de fardaria: {periodo['planet']} en {periodo['sign']}"
                })
                
                # Incluimos también subperiodos principales (nivel 2)
                for subperiodo in periodo.get('subPeriods', []):
                    liberaciones.append({
                        "fecha": subperiodo['start'],
                        "planeta_liberador": subperiodo['planet'],
                        "planeta_liberado": periodo['planet'],
                        "punto": "FARDARIAS",
                        "angulo": 0,  # Valor arbitrario
                        "fuerza": PLANET_DATA[subperiodo['planet']]['numero'],
                        "signo": subperiodo.get('sign', periodo['sign']),
                        "regente": subperiodo.get('regente', periodo['regente']),
                        "condicion": "Inicio de Subperiodo",
                        "descripcion": f"Inicio de subperíodo de fardaria: {subperiodo['planet']} en {periodo['planet']}"
                    })
        else:
