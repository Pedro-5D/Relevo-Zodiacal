...
# Funciones exclusivas para Fardarias
def fardaria_calculate_date(birth_date, day_offset):
    """Función específica de Fardarias para calcular fechas."""
    # Siempre convertir a datetime primero para operaciones
    if isinstance(birth_date, str):
        try:
            ...
            try:
                ...
                # Fecha por defecto en caso de error
                print(f"Error convirtiendo fecha en fardaria_calculate_date: {birth_date}")
                birth_date_obj = datetime(2000, 1, 1)
    else:
        # Si ya es datetime, usarlo directamente
        birth_date_obj = birth_date
    
    # Calcular nueva fecha
    result_date = birth_date_obj + timedelta(days=int(day_offset))
    
    # Devolver como string en formato consistente
...
def calculate_sub_periods(main_planet, level, start_day, end_day, birth_date, planet_order):
    ...
    for planet in rotated_planets:
        ...
        if actual_duration > 0:
            # MODIFICACIÓN IMPORTANTE: Asegurar que start_date y end_date sean strings
    ...
    while current_day < end_day:
        for planet in planet_order:
            ...
            if period_duration > 0:
                # MODIFICACIÓN IMPORTANTE: Asegurar que start_date y end_date sean strings
                # Usamos la función calculate_date que devuelve un string
                start_date = calculate_date(birth_date, period_start_day)
                end_date = calculate_date(birth_date, period_end_day)
                
                # Ahora que tenemos la fecha como string, podemos volver a convertirla para análisis
                # Pero solo si necesitamos trabajar con ella como objeto datetime
                try:
                    start_date_obj = datetime.strptime(start_date, '%d/%m/%Y')
                    month = start_date_obj.month
                except:
                    # Si hay un error, usar el mes 1 como predeterminado
                    month = 1
...
def calculate_forecasts():
    ...
    try:
        ...
        # Para el punto de referencia FARDARIAS, no calculamos liberaciones normales
        if punto_referencia == 'FARDARIAS':
            # Creamos "liberaciones" basadas en los periodos de fardarias (para compatibilidad con frontend)
            liberaciones = []
